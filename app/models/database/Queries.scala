...
object Queries extends GeneticsDbTables {
  ...
  def credibleSetByChrPosRefAltStdId(v: Variant,
                                     studyId: String
  ): Query[FRM.CredSet, Entities.CredSetRow, Seq] = credsets.filter(r =>
  ...
  def colocOnVariantAndStudy(v: Variant,
                             studyId: String
  ): Query[FRM.Coloc, Entities.ColocRow, Seq] = colocs
    .filter(r =>
